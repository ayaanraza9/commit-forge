...
function findConfigFile() {
	...
	// Only search in the project root directory
...
function loadTypeScriptFile(filePath) {
	...
	try {
		...
		// Clear cache for both absolute path and resolved path
...
function loadConfigFile(filePath) {
	...
	if (ext === ".js") {
		...
		// Also try to delete by resolved path
		try {
			const resolved = require.resolve(absolutePath);
			if (require.cache[resolved]) {
				delete require.cache[resolved];
	...
	if (ext === ".ts") {
		...
		return loadTypeScriptFile(absolutePath);
	}
...
function getConfig() {
	...
	try {
		// Resolve to absolute path to ensure it works from any location
		...
		const userConfig = loadConfigFile(absoluteConfigPath);
