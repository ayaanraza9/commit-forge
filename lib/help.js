...
function printHelp() {
  const usage = [
    `${sectionTitle('Usage')}`,
    `  ${colorize('commitly generate --jira TB-123 --category feat --message "add login" --description "Implement login flow"', COLORS.dim)}`,
    `  ${colorize('commitly lint .git/COMMIT_EDITMSG', COLORS.dim)}`,
    `  ${colorize('commitly commit', COLORS.dim)}`,
  ];
  ...
  const sections = [
    `${colorize('commitly', `${COLORS.bold}${COLORS.cyan}`)} - generate, lint, or create commits with enforced formatting`,
    '',
    ...usage,
    '',
    ...commands,
