...
function printHelp() {
  const usage = [
    `${sectionTitle('Usage')}`,
    `  ${colorize('commit-mate generate --jira TB-123 --category feat --message "add login" --description "Implement login flow"', COLORS.dim)}`,
    `  ${colorize('commit-mate lint .git/COMMIT_EDITMSG', COLORS.dim)}`,
    `  ${colorize('commit-mate commit', COLORS.dim)}`,
  ];
  ...
  const sections = [
    `${colorize('commit-mate', `${COLORS.bold}${COLORS.cyan}`)} - generate, lint, or create commits with enforced formatting`,
    '',
    ...usage,
    '',
    ...commands,
